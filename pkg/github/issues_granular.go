...
// reactionResponse is the response type for reaction tools.
type reactionResponse struct {
	ID      int64  `json:"id"`
	Content string `json:"content"`
}

// GranularAddIssueReaction creates a tool to add a reaction to an issue.
func GranularAddIssueReaction(t translations.TranslationHelperFunc) inventory.ServerTool {
	st := NewTool(
		ToolsetMetadataIssues,
		mcp.Tool{
			Name:        "add_issue_reaction",
			Description: t("TOOL_ADD_ISSUE_REACTION_DESCRIPTION", "Add a reaction to a GitHub issue."),
			Annotations: &mcp.ToolAnnotations{
...
// GranularAddIssueCommentReaction creates a tool to add a reaction to an issue comment.
func GranularAddIssueCommentReaction(t translations.TranslationHelperFunc) inventory.ServerTool {
	st := NewTool(
		...
		mcp.Tool{
			...
			Description: t("TOOL_ADD_ISSUE_COMMENT_REACTION_DESCRIPTION", "Add a reaction to a GitHub issue comment."),
			Annotations: &mcp.ToolAnnotations{
			...
			InputSchema: &jsonschema.Schema{
				...
				Properties: map[string]*jsonschema.Schema{
					"owner":      {Type: "string", Description: "Repository owner (username or organization)"},
					"repo":       {Type: "string", Description: "Repository name"},
					"comment_id": {Type: "number", Description: "The ID of the issue comment to react to"},
					"content": {
						Type:        "string",
						Description: "The reaction type",
						Enum:        []any{"+1", "-1", "laugh", "confused", "heart", "hooray", "rocket", "eyes"},
